...
export function composeProposal(input: Proposal, options: CompositionOptions = {}): CompositionResult {
    ...
    blocks.push(...composeTechnicalSections(items.filter(item => !separateIds.has(item.id)), p.companies.map(company => company.id), p.options.detailLevel, options));
    ...
    return {
        ok: true, issues,
        document: { schemaVersion: 2, compositionVersion: 3, stage: 'composed', metadata: p.metadata, client: p.client, isGroup: p.isGroup ?? p.client.kind === 'group', ...(p.client.kind === 'group' ? { groupName: p.groupName ?? p.client.displayName } : {}), locale: 'pt-BR', currency: 'BRL', theme: 'engmarq', sourceItems: items, blocks }
    };
}
